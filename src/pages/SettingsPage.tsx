...
function ThemeSection({ onOpenCustomModal }: ThemeSectionProps) {
  const { t } = useTranslation();
      ...
      <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
        {PREDEFINED_THEMES.map((pt) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="w-1/2 h-full" style={{ background: pt.preview.surface }} />
                <div className="flex-1 flex items-center justify-center">
                  <div className="w-3 h-3 rounded-full" style={{ background: pt.preview.accent }} />
                </div>
              </div>
              <span
        ...
        >
          <div
            ...
            style={{
              background: "linear-gradient(135deg, #6c8cff 0%, #f46c6c 33%, #4caf6c 66%, #ef8c38 100%)",
            }}
          />
          <span
            className="text-xs font-medium"
...
function TagsSection({ tags, deletingId, deleteError, onEdit, onDelete }: TagsSectionProps) {
  const { t } = useTranslation();
        ...
        >
          {tags.map((tag) => (
            <div key={tag.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <span className="nexus-tag" style={{ borderColor: tag.color, color: tag.color }}>
                {tag.name}
              </span>
              ...
              <button
                ...
                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(244,108,108,0.08)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
              >
...
export function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <LanguageSection />
      <ThemeSection onOpenCustomModal={() => setShowCustomModal(true)} />
      <TagsSection
        tags={tags}
